...
(async () => {
  try {
    ...
    console.table(before.map(u => ({email: u.email, role_id: String(u.role_id), password_hash: u.password_hash.slice(0,20) + '...'})));
    ...
    // Cycle 3: Logout simulation and attempt login user
    ...
    console.table(after.map(u => ({email: u.email, role_id: String(u.role_id), password_hash: u.password_hash.slice(0,20) + '...'})));

    // Check if any password_hash changed for admin or user
    ...
    ['admin@cmc.ma','user@cmc.ma'].forEach(email => {
      ...
      if (!b) console.log('  Before: MISSING'); else console.log('  Before:', b.slice(0,20)+'...');
      if (!a) console.log('  After: MISSING'); else console.log('  After :', a.slice(0,20)+'...');
